# Simple Health API Service for PostgreSQL and Kafka
param([int]$Port = 8087)

Write-Host "Starting Health Check API on port $Port" -ForegroundColor Green
...
function Test-Kafka {
    try {
        ...
        $consumerGroups = docker logs kafka-testnet --tail 10 2>$null | Select-String "GroupCoordinator"
        if ($consumerGroups) {
