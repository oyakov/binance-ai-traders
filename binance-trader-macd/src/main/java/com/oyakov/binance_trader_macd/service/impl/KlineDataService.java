...
import com.oyakov.binance_trader_macd.repository.elastic.KlineElasticRepository;
import com.oyakov.binance_trader_macd.repository.jpa.KlinePostgresRepository;
...
public class KlineDataService implements KlineDataServiceApi {
    ...
    private final List<CrudRepository<OrderItem, Long>> repositories = new ArrayList<>();
    ...
    public void saveKlineData(KlineCollectedCommand command) {
        ...
        OrderItem kline = OrderItem.builder()
        ...
        try {
            for (CrudRepository<OrderItem, Long> repository : repositories) {
                log.info("Saving kline data to repository: {}", repository);
                repository.save(kline);
            }
            log.info("Kline data saved: {}", kline);
            eventPublisher.publishEvent(DataItemWrittenNotification.<OrderItem>builder()
    ...
    @Override
    public void compensateKlineData(DataItemWrittenNotification<OrderItem> event) {
        for (CrudRepository<OrderItem, Long> repository : repositories) {
